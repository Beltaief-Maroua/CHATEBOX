...
function StripePayment() { 
    const [product, setProduct] = useState({ 
      ...
      const makePayment = async () => { 
        const stripe = await loadStripe("pk_test_51MMpjZChi0gDpptvpvJC6MjHoHuLwogXAKP4BMtfQ2iqIMVCeQVgHUul3OnnEYhrgeQsftnvu1LnGaN99XrDTLqj00c2KK2pVi"); 
        const body = { product }; 
        const headers = { 
        ...
        const response = await fetch( 
          "http://localhost:3001/api/paiement", 
          { 
            method: "POST", 
            headers: headers, 
